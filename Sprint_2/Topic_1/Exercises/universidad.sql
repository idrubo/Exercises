 ...
 * del rango de los departamentos. Esto es, se han añadido sentencias "COSTRAINT"
 * al efecto.
 */
...
INSERT INTO profesor VALUES (25, 10); --   Correcto.
...
-- 2.4
SELECT persona.apellido1, persona.apellido2, persona.nombre FROM persona JOIN profesor ON persona.id=profesor.id_profesor LEFT JOIN asignatura ON profesor.id_profesor=asignatura.id_profesor WHERE asignatura.id_profesor IS NULL;
...
-- 2.6
/* Los departamentos buscados han de ser aquellos que no tienen pofesor asignado, por: */
-- SELECT DISTINCT asignatura.nombre, alumno_se_matricula_asignatura.id_curso_escolar, departamento.nombre FROM asignatura LEFT JOIN alumno_se_matricula_asignatura ON asignatura.id=alumno_se_matricula_asignatura.id_asignatura LEFT JOIN profesor ON asignatura.id_profesor=profesor.id_profesor LEFT JOIN departamento ON profesor.id_profesor=departamento.id WHERE alumno_se_matricula_asignatura.id_curso_escolar IS NULL;

SELECT departamento.nombre FROM departamento WHERE departamento.nombre<>(SELECT DISTINCT departamento.nombre FROM profesor RIGHT JOIN departamento ON profesor.id_departamento=departamento.id WHERE profesor.id_profesor IN (SELECT DISTINCT asignatura.id_profesor FROM asignatura WHERE asignatura.id_profesor IS NOT NULL));

-- 3.1
SELECT COUNT(asignatura.id) FROM asignatura;

-- 3.2
SELECT COUNT(id) FROM persona WHERE tipo='alumno' AND YEAR(fecha_nacimiento)=1999;

-- 3.3
SELECT departamento.nombre, COUNT(profesor.id_profesor) AS profesores FROM profesor JOIN departamento ON profesor.id_departamento=departamento.id GROUP BY profesor.id_departamento ORDER BY profesores;

-- 3.4
SELECT departamento.nombre, COUNT(profesor.id_profesor) AS profesores FROM departamento LEFT JOIN profesor ON profesor.id_departamento=departamento.id GROUP BY departamento.id ORDER BY profesores;

-- 3.5
SELECT grado.nombre, COUNT(asignatura.id) AS asignaturas FROM asignatura RIGHT JOIN grado ON grado.id=asignatura.id_grado GROUP BY grado.id ORDER BY asignaturas DESC;

-- 3.6
SELECT grado.nombre, COUNT(asignatura.id) AS asignaturas FROM asignatura RIGHT JOIN grado ON grado.id=asignatura.id_grado GROUP BY grado.id HAVING COUNT(asignatura.id) > 40 ORDER BY asignaturas DESC;

-- 3.7
SELECT grado.nombre, asignatura.tipo, SUM(asignatura.creditos) AS creditos FROM asignatura RIGHT JOIN grado ON asignatura.id_grado=grado.id GROUP BY asignatura.tipo, grado.id;
