...
-- 1.5
SELECT  id, nombre, creditos AS "cr", tipo, curso, cuatrimestre, id_profesor, id_grado
  FROM asignatura WHERE id_grado = 7 AND curso = 3 AND cuatrimestre = 1;
...
SELECT DISTINCT departamento.nombre FROM asignatura
  JOIN profesor ON asignatura.id_profesor=profesor.id_profesor
	...
	JOIN grado ON asignatura.id_grado=grado.id
 	...
 	AND asignatura.id_profesor IS NOT NULL;
...
-- 3.8
SELECT YEAR(anyo_inicio) AS anyo, COUNT(DISTINCT(id_alumno)) AS alumnos FROM alumno_se_matricula_asignatura
  JOIN curso_escolar ON alumno_se_matricula_asignatura.id_curso_escolar=curso_escolar.id
